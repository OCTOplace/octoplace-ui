...
import { updateSpotData } from "../hook/useGrid";
        ...
        return new Promise(async (resolve) => {
            ...
            staticProvider.on(
                ...
                (log) => { // executed if event gets caught example loading ends
                    const [tokenId] = ethers.utils.defaultAbiCoder.decode(['uint256'], log.topics[1]);
                    const [owner] = ethers.utils.defaultAbiCoder.decode(['address'], log.topics[2]);
                    const [x, y, width, height, title, image, link, updated] = ethers.utils.defaultAbiCoder.decode(ABI_DECODE, log.data);
                    const spot = {
                        x, y, width, height, title, image, link, owner,
                        _index: tokenId.toNumber(),
                    };
                    updateSpotData(spot)
                    if (owner.toLowerCase() === signer.getAddress().toLowerCase() && log.blockNumber >= currentHeight) {
...
const contractInteraction = {
    ...
    gatherSpots: async () => {
        ...
        return { spotsLength, isGridLoading }
    },
