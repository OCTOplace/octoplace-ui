...
export const MyListedNFT = (props) => {
    ...
    useEffect(() => {
        if(listings.length > 0){
            ...
            const myListings = active.filter(item=> (item.listingDetails.tokenOwner === account));
    ...
    return (
        <Grid container spacing={2}>
                ...
                {(account && userListings.length === 0 && !loading) && (
                  <div style={{textAlign: "center", width: "100%", marginTop: "10vh"}}>
                  <Typography variant="h4" sx={{color:"grey", textAlign: "center"}}>You have not listed any NFT's.</Typography>
                </div>
                )}
                {view !== 1 &&
